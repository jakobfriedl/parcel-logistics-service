...
public class TransitionValidator : AbstractValidator<Parcel>
{
    public TransitionValidator()
    {
        ...
        RuleFor(senderCountry => senderCountry.Sender.Country).NotNull();

        When(senderCountry => senderCountry.Sender.Country == "Austria" || senderCountry.Sender.Country == "Österreich", () =>
        {
            //only Checks if Country is Austria|Österreich
...
public class TransitionLogic : ITransitionLogic
{
    ...
    public Parcel TransitionParcel(string trackingId, Parcel parcel)
    {
        ...
        DataAccess.Entities.Parcel parcelById = null; 
        
        if(_parcelRepository.TryGetByTrackingId(trackingId, out parcelById)){
